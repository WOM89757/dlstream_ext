...
struct GalleryObject {
    ...
    GalleryObject(const std::vector<cv::Mat>& embeddings,
                  const std::string& label, int id, std::string personId)
        : embeddings(embeddings), label(label), id(id), personId(personId) {}
};
...
public:
    ...
    std::vector<int> GetGalleryObjectByEmbeddings(const std::vector<cv::Mat>& embeddings);
    std::string GetLabelByID(int id) const;
    std::string GetPersonIdByID(int id) const;
    float GetThresholdByID(int id) const;
