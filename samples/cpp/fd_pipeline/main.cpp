...
void AlignFaces(std::vector<cv::Mat>* face_images,
    ...
    for (size_t j = 0; j < face_images->size(); j++) {
        for (int i = 0; i < ref_landmarks.rows; i++) {
            std::cout << ref_landmarks_normalized[2 * i] << " " << ref_landmarks_normalized[2 * i] << std::endl;
    ...
    for (GVA::RegionOfInterest &roi : video_frame.regions()) {
        ...
        for (auto tensor : roi.tensors()) {
            ...
            if (layer_name == "658") {
                ...
                auto ids  = fr->GetIDsByEmbeddingsWithThreshold(embeddings);
                for (int i = 0; i < ids.size(); i++) {
                //    std::cout << "id: " <<  ids[i] << " " << std::endl;
                   std::string personId = fr->GetPersonIdByID(ids[i]);
                   personlabel = fr->GetLabelByID(ids[i]);
                   float threshold = fr->GetThresholdByID(ids[i]);
                //    std::cout << "personlabel: " <<  personlabel << " " << std::endl;
...
int main(int argc, char *argv[]) {
    ...
    auto launch_str = g_strdup_printf("%s=%s ! %s ! %s ! "
                                      "gvadetect model=%s device=%s batch-size=%d model-proc=/home/zy/opvinomodels/model_proc/intel/object_detection/face-detection-retail-0004.json  ! queue ! "
                                    //   "%s"
                                    "gvaclassify model=/home/zy/opvinomodels/intel/landmarks-regression-retail-0009/FP32/landmarks-regression-retail-0009.xml model-proc=/home/zy/intel/dlstreamer_gst/samples/model_proc/intel/object_attribute_estimation/landmarks-regression-retail-0009.json device=CPU batch-size=1 inference-region=roi-list object-class=face ! queue name=gvainference_queue1 ! "
                                    "gvaclassify pre-process-backend=opencv model=/home/zy/opvinomodels/intel/face-reidentification-retail-0095/FP32/face-reidentification-retail-0095.xml model-proc=/home/zy/intel/dlstreamer_gst/samples/model_proc/intel/object_attribute_estimation/face-reidentification-retail-0095.json device=CPU batch-size=1 inference-region=roi-list object-class=face ! queue name=gvametaconvert_queue ! "
...
// filesrc location=/home/zy/3.mp4 ! decodebin ! videoconvert n-threads=4 ! videoscale n-threads=4  ! 
// capsfilter caps="video/x-raw,format=BGRA" ! 
// gvadetect model=/home/zy/opvinomodels/intel//face-detection-adas-0001/FP32//face-detection-adas-0001.xml 
// device=CPU batch-size=1 model-proc=/home/zy/opvinomodels/model_proc/intel/object_detection/face-detection-retail-0004.json  ! 
// queue ! gvainference model=/home/zy/opvinomodels/intel//face-reidentification-retail-0095/FP32//face-reidentification-retail-0095.xml 
// device=CPU batch-size=1 inference-region=roi-list ! queue name=gvainference_queue1 ! 
// gvainference model=/home/zy/opvinomodels/intel//landmarks-regression-retail-0009/FP32//landmarks-regression-retail-0009.xml 
// device=CPU batch-size=1 inference-region=roi-list ! queue name=gvametaconvert_queue !
//  gvametaconvert json-indent=4 ! gvametapublish method=stdout ! queue ! 
//  gvawatermark name=gvawatermark ! videoconvert n-threads=4 ! identity signal-handoffs=false ! 
...
//  fpsdisplaysink sync=false
    ...
    std::string fd_model_path = "/home/zy/opvinomodels/intel/face-detection-adas-0001/FP32/face-detection-adas-0001.xml";
    std::string lm_model_path = "/home/zy/opvinomodels/intel/landmarks-regression-retail-0009/FP32/landmarks-regression-retail-0009.xml";
    std::string fr_model_path = "/home/zy/opvinomodels/intel/face-reidentification-retail-0095/FP32/face-reidentification-retail-0095.xml";
    std::string FLAGS_fg ="/home/zy/opvinomodels/fdimgs";
    // std::string FLAGS_fg ="/home/zy/opvinomodels/faces_gallery.json";
    ...
    if (!fd_model_path.empty() && !fr_model_path.empty() && !lm_model_path.empty()) {
        ...
        face_registration_det_config.deviceName = "CPU";
        face_registration_det_config.ie = ie;
        face_registration_det_config.is_async = false;
        face_registration_det_config.confidence_threshold = static_cast<float>(0.6);
        face_registration_det_config.increase_scale_x = static_cast<float>(1.15);
        face_registration_det_config.increase_scale_y = static_cast<float>(1.15);

        CnnConfig reid_config(fr_model_path);
        reid_config.deviceName = "CPU";
        reid_config.max_batch_size = 1;
        reid_config.ie = ie;

        CnnConfig landmarks_config(lm_model_path);
        landmarks_config.deviceName =  "CPU";
        ...
        face_recognizer.reset(new FaceRecognizerDefault(
            ...
            FLAGS_fg, 0.7, 128, false, false));

        // face_recognizer = new FaceRecognizerDefault(
        //     landmarks_config, reid_config,
        //     face_registration_det_config,
        //     FLAGS_fg, 0.7, 128, false, false);

    } else {
        std::cout << "not init model path" << std::endl;
        return 0;
    }
    ...
    gst_pad_add_probe(gvametaconvert_queue_pad, GST_PAD_PROBE_TYPE_BUFFER, gvametaconvert_queue_pad_probe_callback, face_recognizer.get(), NULL);
