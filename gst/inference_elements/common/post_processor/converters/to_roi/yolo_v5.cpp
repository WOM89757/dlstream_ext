/*******************************************************************************
 * Copyright (C) 2021 Zy Corporation
 *
...
std::vector<float> YOLOv5Converter::softmax(const float *arr, size_t size, size_t common_offset, size_t side) const {
...
void YOLOv5Converter::parseOutputBlob(const InferenceBackend::OutputBlob::Ptr &blob,
    ...
    for (size_t i = 0; i < side_square; ++i) {
        ...
        for (size_t bbox_cell_num = 0; bbox_cell_num < output_shape_info.bbox_number_on_cell; ++bbox_cell_num) {
            ...
            const float width = std::pow((output_sigmoid_activation ? sigmoid(raw_w) : raw_w) * 2 ,2) * anchors[anchor_offset + 2 * bbox_cell_num];
            const float height = std::pow((output_sigmoid_activation ? sigmoid(raw_h) : raw_h) * 2 ,2) * anchors[anchor_offset + 2 * bbox_cell_num + 1];
...
TensorsTable YOLOv5Converter::convert(const OutputBlobs &output_blobs) const {
    ...
    try {
        ...
        if (model_input_image_info.batch_size != 1) {
            throw std::invalid_argument("Batch size other than 1 is not supported for this post processor: " +
                                        std::string(YOLOv5Converter::getName()));
        }
...
size_t YOLOv5Converter::entryIndex(size_t side, size_t location, size_t entry) const {
    size_t side_square = side * side;
    size_t bbox_cell_num = location / side_square;
    size_t loc = location % side_square;
    // side_square is the tensor dimension of the YoloV5 model. Overflow is not possible here.
