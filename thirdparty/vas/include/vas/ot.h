...
public:
     ...
     */
    DetectedObject(const cv::Rect& input_rect, int32_t input_class_label) : rect(input_rect), class_label(input_class_label) {}
 ...
 * For each frame, detected objects are mapped to tracked objects with this tracking type, which enables ID tracking of detected objects.
 * @n
...
public:
     ...
     * If a detected object is overlapped enough with one of tracked object, the tracked object's information is updated with the input detected object.
     * On the other hand, if a detected object is overlapped with none of tracked objects, the detected object is newly added and ObjectTracker starts to track the object.
     * In long term and short term tracking type, ObjectTracker continues to track objects in case that empty list of detected objects is passed in.
     * In zero term tracking type, however, ObjectTracker clears tracked objects in case that empty list of detected objects is passed in.
     * @n
     ...
     */
    VAS_EXPORT std::vector<Object> Track(const cv::Mat& frame, const std::vector<DetectedObject>& detected_objects = std::vector<DetectedObject>());
    #ifdef BUILD_DPCPP
    VAS_EXPORT std::vector<Object> TrackGPU(VASurfaceID surface_id, uint64_t width, uint64_t height, const std::vector<DetectedObject>& detected_objects = std::vector<DetectedObject>());
    #endif
     ...
     * If input frames come from a video stream of constant frame rate, then a user needs to set this value as 1.0/fps just after video open. For example, 60 fps video stream should set 0.0167f.
     * If input frames have inconstant frame rate, then a user needs to call this function before the Track() function.
     *
     * @param[in] frame_delta_t Delta time between two consecutive tracking in seconds. The valid range is [0.005 ~ 0.5].
     */
...
#ifdef BUILD_DPCPP
    VAS_EXPORT std::unique_ptr<ObjectTracker> Build(VADisplay display, TrackingType tracking_type) const;
#endif
...
public:
     ...
     * @n
     * Valid range: 1 <= max_num_objects. Or it can be -1 if there is no limitation of maximum number in X86. KMB/TBH has limitation up to 1024.
     * @n
     ...
     * For example, when detector confuses bicycle and motorbike. Turning this option off will increase the tracking reliability as tracker will ignore the class label of detector.
     * @n
...
     * @n
     * For Parallelization in KCFVAR mode, use key "max_num_threads" to set the maximum number of threads. Consult the following format
